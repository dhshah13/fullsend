...
func TestWarnRepoSkillCollisions_DoesNotWarnWithoutCollision(t *testing.T) {
	...
	// A matching directory without SKILL.md is not a discoverable repo skill.
	require.NoError(t, os.MkdirAll(filepath.Join(projectSkillsDir, "not-a-skill"), 0o755))

	harnessRoot := t.TempDir()
	harnessSkills := []string{
		filepath.Join(harnessRoot, "harness-only"),
		filepath.Join(harnessRoot, "not-a-skill"),
	}
