...
- Level 3 content capture activates only under a dual-consent, allow-listed, fail-closed contract — operator env opt-in, per-agent harness consent, and an org-level endpoint allow-list whose entries record governance sign-off ([ADR 0084](ADRs/0084-level-3-content-capture-activation-contract.md)). A sandbox environment denylist prevents in-sandbox self-enablement of the runtime's native content telemetry ([ADR 0085](ADRs/0085-sandbox-environment-variable-denylist.md)).
