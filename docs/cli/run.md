...
The plan block prints `Runtime: <name> (from <source>)` and, when an override applied, `Model: <value> (from <source>)`; stderr carries `runtime: selected "<name>" from <source>` (and `model: requested "<value>" from <source>`) for scripts. A value from the config file is labelled with the file path, suffixed ` agents.<name>` when the agent's entry decided. When `models.aliases` in `.fullsend/config.yaml` remaps the alias, the line becomes `Model: sonnet (from <source>) → claude-sonnet-5 (from <config path> models.aliases)` and stderr adds `model: alias "sonnet" remapped to "claude-sonnet-5" from <config path> models.aliases`. Each aliased entry in the `Fallback models` line is shown as `alias → id` in the compact chain (`sonnet → claude-sonnet-5, claude-opus-4-6 (from FULLSEND_FALLBACK_MODELS)`) — no per-entry source and no stderr line; literal ids are printed as written, and pi ignores the chain with a warning regardless. An invalid override (unknown runtime, unknown effort level, an `agents:` entry that names no agent, a `models.aliases` key outside `opus`/`sonnet`/`haiku`/`fable`, or a value that is not a model id or `provider/id` — including another alias name) fails before the sandbox is created.
