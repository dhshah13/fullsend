...
### Mapping scope: repository only, no ref by default

The OpenAI mapping asserts `iss`, `aud`, and `repository` — matching the
Vertex path's `attribute.repository` scoping
(`internal/dispatch/gcf/provisioner.go`, `buildAttributeCondition`). No
`ref` assertion is generated by default (#6782). The finer policy (who
may trigger an agent, which workflows, which refs) lives in fullsend's
dispatch authorization and the mint's prevalidation.

`--ref` is an explicit **tightening** option: when set, it emits two
mappings per repository — one for the named ref and one for
`refs/pull/*` — because `pull_request_review` events carry
`refs/pull/<N>/merge`, not the default branch ref. OpenAI allows one
trailing wildcard with a non-empty prefix; assertions within a mapping
are AND-ed, and OR-ed across mappings (50 mappings per provider). The
two-mapping cost halves the per-provider budget to 25 repositories.
...
- #6782 — drop ref assertion by default
