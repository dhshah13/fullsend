...
- **[Debugging](debugging.md)** — Observability provides the data; debugging provides the methodology — what fault categories to distinguish and how to walk the provenance chain. The [agents skipping instructions](debugging.md#agents-skipping-instructions) analysis identifies a class of faults where trace inspection alone is insufficient; detecting them requires comparing traces against the agent's full instruction set.
