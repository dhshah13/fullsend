...
| [Go Code](docs/contributing/go-code.md) | Changing Go code under `cmd/`, `internal/`, or `pkg/` — covers mint sync, interface documentation sync, coverage, vet, e2e tests, concurrency testing, context-aware blocking, suite-timeout policy, WASM binary size constraints, secure HTTP client construction, and preferring `go run` for the CLI |
